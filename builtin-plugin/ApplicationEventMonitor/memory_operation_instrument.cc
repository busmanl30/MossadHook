...
void malloc_handler(MossadRegisterContext *ctx, const InterceptEntry *info) {
  size_t size_ = 0;
...
void free_handler(MossadRegisterContext *ctx, const InterceptEntry *info) {
  uintptr_t mem_ptr;
...
__attribute__((constructor)) static void ctor() {
  //    MossadInstrument((void *)mmap, malloc_handler);
  //    MossadInstrument((void *)free, free_handler);
  return;
}
