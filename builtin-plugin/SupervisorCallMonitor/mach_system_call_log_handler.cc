...
static addr_t getCallFirstArg(MossadRegisterContext *ctx) {
  addr_t result;
...
static addr_t getRealLr(MossadRegisterContext *ctx) {
  addr_t closure_trampoline_reserved_stack = ctx->sp - sizeof(addr_t);
  return *(addr_t *)closure_trampoline_reserved_stack;
}

static addr_t fast_get_caller_from_main_binary(MossadRegisterContext *ctx) {
...
static void mach_syscall_log_handler(MossadRegisterContext *ctx, const InterceptEntry *info) {
