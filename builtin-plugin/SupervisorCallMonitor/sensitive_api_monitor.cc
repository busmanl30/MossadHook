...
static void sensitive_api_handler(MossadRegisterContext *ctx, const InterceptEntry *info) {
  char buffer[256] = {0};
...
void supervisor_call_monitor_register_sensitive_api_handler() {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    func_addr = (addr_t)MossadSymbolResolver("libsystem_kernel.dylib", func_name);
    if (func_addr == 0) {
      func_addr = (addr_t)MossadSymbolResolver("libsystem_kernel.dylib", sensitive_func_array[i]);
    }
    if (func_addr == 0) {
  ...
  // ===============
  MossadHook((void *)sysctl, (void *)fake_sysctl, (void **)&orig_sysctl);
