...
static void common_supervisor_call_monitor_handler(MossadRegisterContext *ctx, const InterceptEntry *info) {
...
void supervisor_call_monitor_register_svc(addr_t svc_addr) {
  ...
  MossadInstrument((void *)svc_addr, common_supervisor_call_monitor_handler);
...
void supervisor_call_monitor_register_system_kernel() {
  ...
  addr_t write_svc_addr = (addr_t)MossadSymbolResolver("libsystem_kernel.dylib", "write");
  write_svc_addr += 4;

  addr_t __psynch_mutexwait_svc_addr = (addr_t)MossadSymbolResolver("libsystem_kernel.dylib", "__psynch_mutexwait");
