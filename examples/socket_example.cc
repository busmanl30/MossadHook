...
#define install_hook(name, fn_ret_t, fn_args_t...)                                                                     \
  ...
  /* __attribute__((constructor)) */ static void install_hook_##name() {                                               \
    void *sym_addr = MossadSymbolResolver(NULL, #name);                                                                 \
...
void common_handler(void *address, MossadRegisterContext *ctx) {
...
__attribute__((constructor)) static void ctor() {
  ...
  for (int i = 0; i < sizeof(func_array) / sizeof(char *); ++i) {
    func = MossadSymbolResolver(NULL, func_array[i]);
    if (func == NULL) {
  ...
  for (auto iter = func_map->begin(), e = func_map->end(); iter != e; iter++) {
    ...
    if (is_short) {
      ...
      MossadInstrument(iter->first, common_handler);
      break;
    }
  }

#if defined(__APPLE__)
  // MossadImportTableReplace(NULL, "_pthread_create", (void *)fake_pthread_create, (void **)&orig_pthread_create);
#endif
