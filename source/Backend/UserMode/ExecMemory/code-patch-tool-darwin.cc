...
PUBLIC int MossadCodePatch(void *address, uint8_t *buffer, uint32_t buffer_size) {
  ...
  if ((addr_t)address + buffer_size > patch_page + page_size) {
    ...
    auto ret = MossadCodePatch(address_a, buffer_a, buffer_size_a);
    if (ret == -1) {
      return ret;
    }
    ...
    ret = MossadCodePatch(address_b, buffer_b, buffer_size_b);
    return ret;
  }
  ...
  if (0 && is_enable_remap == 1) {
    ...
    if (vm_protect_fn == nullptr) {
      vm_protect_fn = (__typeof(vm_protect) *)MossadSymbolResolver("dyld", "vm_protect");
      if (vm_protect_fn == nullptr) {
        vm_protect_fn = (__typeof(vm_protect) *)MossadSymbolResolver("libsystem_kernel.dylib", "_vm_protect");
      }
