...
struct InstrumentRouting : InterceptRouting {
  ...
  InstrumentRouting(Interceptor::Entry *entry, mossad_instrument_callback_t pre_handler) : InterceptRouting(entry) {
  }
...
PUBLIC inline int MossadInstrument(void *address, mossad_instrument_callback_t pre_handler) {
  __FUNC_CALL_TRACE__();
  if (!address) {
  ...
  DEBUG_LOG("----- [MossadInstrument: %p] -----", address);
