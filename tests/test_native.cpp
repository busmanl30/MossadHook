...
uint64_t get_arg(MossadRegisterContext *ctx, int index) {
...
void test_dlopen() {
  ...
  {
    auto sym_addr = MossadSymbolResolver("dyld", "_dlopen");
...
void test_execve() {
  ...
  {
    auto sym_addr = MossadSymbolResolver(NULL, "_execve");
    LOG("execve: %p", sym_addr);
    MossadInstrument(sym_addr, [](void *, MossadRegisterContext *ctx) {
